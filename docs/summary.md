...
Return to [index](https://github.com/marchandd/term_ssh_wine_winmerge/ "Index")
...
- Run one same `winmerge.sh` command in cyclic mode to run sub programs:  
One run to set Wine emulator, one run to install and one run to launch.  
- winmerge alias is not available at the end of progam. You have to make alias activation later.
...
- :computer: `winmerge.sh` to set Wine emulator.  
- :computer: `winmerge.sh` to install programm.  
- :computer: `winmerge.sh` to launch programm.  
